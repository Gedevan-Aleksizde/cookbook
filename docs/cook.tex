...
\hypersetup{
  pdftitle={ザ・クックブック},
  pdfauthor={片桐 智志 (Katagiri, Satoshi, ill-identified)},
  colorlinks=true,
  linkcolor=blue,
  filecolor=Maroon,
  citecolor=blue,
...
\title{ザ・クックブック}
\author{片桐 智志 (Katagiri, Satoshi, ill-identified\footnote{twitter
  ID: @ill-identified})}
...
\tightlist
\item
  (魚をまるごと使う場合) 腹を切り裂きワタとエラを取る.
  大きな魚であれば取り分けやすい大きさに切る
\item
...
玉ねぎを入れる人は多いが, \citet{boumei}
はウハーは透き通ったスープであることが大切なので,
...
庶民の食事の1つだったのかもしれない (図\ref{fig:lamb-sage-finished}).
...
\caption{ラム肉のセージソース和え}\label{fig:lamb-sage-finished}
\end{figure}
